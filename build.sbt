...
lazy val `token-editor` = project
  ...
  .settings(
    assembly / assemblyJarName := "token-editor.jar",
...
lazy val `morphological-engine-ui` = project
  ...
  .settings(
    ...
    assembly / assemblyJarName := "morphological-engine-ui",
    ThisBuild / assemblyMergeStrategy := {
      // case PathList("META-INF", "versions", xs @ _*)       => MergeStrategy.discard
      case PathList("reference.conf")                               => MergeStrategy.concat
      case PathList("logback.xml")                                  => MergeStrategy.last
...
addCommandAlias("te-assembly", "token-editor / clean;token-editor / assembly")
addCommandAlias("te-assembly", "morphological-engine-ui / clean;morphological-engine-ui / assembly")
