...
import conjugation.forms.verb.{ FormI, FormII, FormX }
...
class RootWordSpec extends FunSuite {
  ...
  test("RootWord: transform 4 letter word with shadda") {
    val word =
      FormII.PresentPassiveTense.rootWord.transform(ArabicLetterType.Ain, ArabicLetterType.Lam, ArabicLetterType.Meem)
    assertEquals(word.label, "يُعَلَّمُ")
  }
